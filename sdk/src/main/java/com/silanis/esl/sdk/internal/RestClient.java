...
import com.silanis.esl.sdk.SocksConfiguration;
...
public class RestClient {
	...
	public static final String ESL_API_VERSION = "11.0";
	public static final String ESL_API_VERSION_HEADER = "esl-api-version="
			+ ESL_API_VERSION;

	public static final String CONTENT_TYPE_APPLICATION_JSON = "application/json";
	public static final String ESL_CONTENT_TYPE_APPLICATION_JSON = CONTENT_TYPE_APPLICATION_JSON
	...
	public static final String ESL_ACCEPT_TYPE_APPLICATION_JSON = ACCEPT_TYPE_APPLICATION_JSON
			+ "; " + ESL_API_VERSION_HEADER;
	public static final String ESL_ACCEPT_TYPE_APPLICATION_OCTET_STREAM = ACCEPT_TYPE_APPLICATION_OCTET_STREAM
			+ "; " + ESL_API_VERSION_HEADER;
	public static final String ESL_ACCEPT_TYPE_APPLICATION = ACCEPT_TYPE_APPLICATION
			+ "; " + ESL_API_VERSION_HEADER;
	...
	private SocksConfiguration socksConfiguration;
	...
	public RestClient(String apiToken, boolean allowAllSSLCertificates) {
		this(apiToken, allowAllSSLCertificates, (ProxyConfiguration) null);
	}
	...
	public RestClient(String apiToken, SocksConfiguration socksConfiguration) {
		this(apiToken, false, socksConfiguration);
	}
	...
	public String post(String path, String jsonPayload) throws IOException,
			HttpException, URISyntaxException, RequestException {
	...
	public String postMultipartFile(String path, String fileName,
			byte[] fileBytes, String jsonPayload) throws IOException,
			HttpException, URISyntaxException, RequestException {
		support.logRequest("POST", path, jsonPayload);

		final MultipartEntityBuilder multipartEntityBuilder = MultipartEntityBuilder
				.create();
		multipartEntityBuilder.addPart("payload",
				buildPartForPayload(jsonPayload));
		multipartEntityBuilder.addPart("file",
				buildPartForFile(fileBytes, fileName));
	...
	public String postMultipartPackage(String path,
			Collection<Document> documents, String jsonPayload)
	...
	private StringBody buildPartForPayload(String jsonPayload) {
		return new StringBody(jsonPayload, ContentType.create("text/plain",
				Consts.UTF_8));
	}
	...
	private ByteArrayBody buildPartForFile(Document document) {
		return buildPartForFile(document.getContent(), document.getFileName(),
				document.getName());
	}

	private ByteArrayBody buildPartForFile(byte[] content, String fileName,
			String name) {
	...
	private <T> T execute(HttpUriRequest request, ResponseHandler<T> handler)
		...
		try {
			...
			throw new RequestException(request.getRequestLine().getMethod(),
					request.getRequestLine().getUri(), 500,
		...
		try {
			...
			if (socksConfiguration != null) {
				InetSocketAddress socksaddr = new InetSocketAddress(socksConfiguration.getSocksHost(),socksConfiguration.getSocksPort());
			...
			if (response.getStatusLine().getStatusCode() >= 400) {
				String errorDetails = Streams.toString(response.getEntity()
						.getContent());
				throw new RequestException(
						request.getRequestLine().getMethod(), request
								.getRequestLine().getUri(), response
				...
				if (null != response.getHeaders("Content-Disposition")
						&& response.getHeaders("Content-Disposition").length > 0) {
					String fileName = getFilename(response
							.getHeaders("Content-Disposition")[0].getValue());
					DownloadedFile downloadedFile = (DownloadedFile) handler
							.extract(bodyContent);
	...
	private String getFilename(String disposition) {
		...
		for (String part : parts) {
			...
			if (index > 0) {
				return percentDecode(part.substring(
						index + fileNameTitle.length(), part.length() - 1));
			}
		}
	...
	private CloseableHttpClient buildHttpClient() throws HttpException {
		...
		if (proxyConfiguration != null) {
			if (proxyConfiguration.hasCredentials()) {
				httpClientBuilder
						.setDefaultCredentialsProvider(buildCredentialsConfiguration(proxyConfiguration));
			}

			httpClientBuilder
					.setDefaultRequestConfig(buildProxyConfiguration(proxyConfiguration));
			return httpClientBuilder.build();
			
		} else if (socksConfiguration != null) {
			
			if (socksConfiguration.hasCredentials()) {
				
			}

			Registry<ConnectionSocketFactory> reg = RegistryBuilder.<ConnectionSocketFactory>create()
			        .register("https", new SocksConnectionSocketFactory(SSLContexts.createSystemDefault()))
	...
	private SSLConnectionSocketFactory buildSSLSocketFactory()
		...
		// Disabling all checks that SSL certificate is valid. We are actually
		// calling e-SignLive anyways.
		// Our client library should implicitly trust our e-SignLive server.
		// This also allows testing against
	...
	private RequestConfig buildProxyConfiguration(
			ProxyConfiguration proxyConfiguration) {
		return RequestConfig
				.custom()
				.setProxy(
						new HttpHost(proxyConfiguration.getHost(),
								proxyConfiguration.getPort(),
								proxyConfiguration.getScheme())).build();
	}

	private CredentialsProvider buildCredentialsConfiguration(
			ProxyConfiguration proxyConfiguration) {
		CredentialsProvider credentialsProvider = new BasicCredentialsProvider();
		credentialsProvider.setCredentials(
				new AuthScope(proxyConfiguration.getHost(), proxyConfiguration
						.getPort()),
	...
	public String get(String path, String acceptType) throws IOException,
			HttpException, URISyntaxException, RequestException {
	...
	public DownloadedFile getBytes(String path) throws IOException,
			HttpException, URISyntaxException, RequestException {
	...
	public DownloadedFile getBytesAsOctetStream(String path)
			throws IOException, HttpException, URISyntaxException,
			RequestException {
		return getBytesAsOctetStream(path,
				ESL_ACCEPT_TYPE_APPLICATION_OCTET_STREAM);
	}

	public DownloadedFile getBytesAsOctetStream(String path, String acceptType)
			throws IOException, HttpException, URISyntaxException,
			RequestException {
		support.logRequest("GET", path);
		HttpGet get = new HttpGet(path);
	...
	public String delete(String path) throws HttpException, IOException,
			URISyntaxException, RequestException {
