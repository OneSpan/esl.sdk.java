...
public class SignatureBuilderTest {
    ...
    public void buildWithSpecifiedValues() {
        ...
        Collection<RadioButtonSet> sets = new ArrayList<RadioButtonSet>();
        for ( int i = 0; i < 2; i++ )
            sets.add( RadioButtonSetBuilder.newRadioButtonsForSigner( email ).build() );
        ...
        for ( RadioButtonSet set : sets )
            builder.withRadioButtonSet( set );
        ...
        assertEquals( sets.size(), signature.getRadioSets().size() );
        for ( RadioButtonSet set : sets )
            assertTrue( signature.getRadioSets().contains( set ) );
    }
