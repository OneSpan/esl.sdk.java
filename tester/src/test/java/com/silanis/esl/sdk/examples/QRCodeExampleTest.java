...
public class QRCodeExampleTest {
    ...
    public void verifyResult() {
        ...
        assertThat("First QR code's style was not set correctly.", example.addedQRCode1.getStyle(), is(FieldStyle.QRCODE));
        ...
        assertThat("Second QR code's style was not set correctly.", example.addedQRCode2.getStyle(), is(FieldStyle.QRCODE));
        ...
        for (Field field : modifiedQRCodeList) {
            if (field.getId().equals(example.qrCodeId1)) {
                assertThat("Modified QR code's style was not set correctly.", field.getStyle(), is(FieldStyle.QRCODE));
        ...
        for (Field updatedQRCode : updatedQRCodeList) {
            if (updatedQRCode.getId().equals(example.qrCodeId1)) {
                assertThat("First updated QR code's style was not set correctly.", updatedQRCode.getStyle(), is(FieldStyle.QRCODE));
            ...
            if (updatedQRCode.getId().equals(example.qrCodeId2)) {
                assertThat("Second updated QR code's style was not set correctly.", updatedQRCode.getStyle(), is(FieldStyle.QRCODE));
